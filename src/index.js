...
const resetStart = () => {
	nose_position_turn = 0;
	nose_position_move = 0;
	nose_position_start = 0;
}
...
const cameraDiv = document.getElementById("camera_div").appendChild(resetButton)
resetButton.addEventListener('click', resetStart)
...
const userCanvas = document.getElementById('user_canvas')
...
const animate = async () => {
	...
	let pose
	try{
		if(user_camera.srcObject){
			pose = await net.estimateSinglePose(user_camera)
		}
		...
		if(pose){
			...
			context.beginPath()
			context.arc(nose_position_turn, nose_position_start, 10, 0, 2*Math.PI)
			context.fill()
	
			context.fillStyle = 'red'
			context.beginPath()
			context.arc(nose.position.x, nose.position.y, 5, 0, 2*Math.PI)
			context.fill()
			if( !nose_position_turn && nose.score > 0.001){
				nose_position_turn = nose.position.x;
			}
			// //turns and moves in the increments of 0.025 
			let nose_turn = (nose.position.x - nose_position_turn)/100;
			js.turn = nose_turn;
	
			if( !nose_position_start ){
				nose_position_start = nose.position.y;
			}
	
			if( !nose_position_move ){
				nose_position_move = nose.position.y;
			}
			var move_trigger = 20
			var nose_move = (nose_position_start - nose.position.y)
			if( nose_position_start - nose.position.y > move_trigger ){
				js.forward = 0.025;
			}else{
				js.forward = 0
			}
